...
module.exports = function(God) {
  ...
  var garbageCollect = function(proc_key, cb) {
    var proc = _getProcessById(proc_key.pm2_env.pm_id);

    if (proc.pm2_env.auto_gc && proc.send && typeof(proc.send) === 'function') {
      debug('Running GC for process %s:%s',
            proc.pm2_env.name,
            proc.pm2_env.pm_id);
      proc.send({type:'run:gc'});
    }
  };
